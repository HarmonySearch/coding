...
class MainWindow(QDialog):
    ...
    def create_ui(self):
        ...
        # УПРАВЛЕНИЕ
        parse = QPushButton('ПАРСИМ')    # парсинга 
        parse.clicked.connect(self.pars_page)
        unload = QPushButton('ВЫКЛАДЫВАЕМ')    # выкладка
        unload.clicked.connect(self.unload)

        # ОТЛАДКА
        self.phase = QComboBox()                            # список функций
        self.phase.setModel(self.phase_model)
        debug = QPushButton('ОТЛАДКА')                      # кнопка запуска функций 
        debug.clicked.connect(self.debug)

        # КОНТЕЙНЕР ДЛЯ КНОПОК
        hbox_button = QHBoxLayout()                         # горизонтальный бокс с кнопками
        hbox_button.addWidget(parse)
        hbox_button.addWidget(unload)
        ...
        self.message.insertPlainText('лог сообщений\n')
        self.error = QTextEdit()
        self.error.insertPlainText('лог ошибок\n')
...
##        print('Вакансий в странице ',vacants)
    ...
    ##                print(m,m.group(1))
            ...
            self.con.commit()
            self.message.insertPlainText('работаю\n')
    ...
    def unload(self):
        ...
        self.rubric() # расставляем рубрики
